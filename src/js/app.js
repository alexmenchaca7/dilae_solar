document.addEventListener('DOMContentLoaded', function() {
    ...
    if (ctxLinea) {
        const generarGraficaRendimiento = () => {
        ...
        const observer = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                // Si el canvas de la gráfica es visible en la pantalla
                if (entry.isIntersecting) {
                    // Llama a la función que acabamos de crear
                    generarGraficaRendimiento();
                    // Deja de observar el elemento para no volver a dibujarla y ahorrar recursos
                    observer.unobserve(ctxLinea);
                }
            });
        });

        // Inicia la observación sobre el elemento del canvas
        observer.observe(ctxLinea);
    }
