...
class ActiveRecord {
    ...
    protected static $campoContenidoHtml = 'contenido'; // Asumimos que así se llama en Blog y otros modelos con WYSIWYG
    ...
    protected static function crearObjeto($registro) {
        ...
        foreach($registro as $key => $value) {
            // Asigna directamente todas las columnas del resultado
            $objeto->$key = $value;

            if (property_exists(static::class, 'campoContenidoHtml') && $key === static::$campoContenidoHtml && is_string($value)) {
                $objeto->$key = stripslashes($value);
            }
        }
    ...
    public function sanitizarAtributos() {
        ...
        foreach($atributos as $key => $value) {
            if ($value === null) { // Solo convertir null explícito a null para la BD
                $sanitizado[$key] = null;
            } 
            else {
                $sanitizado[$key] = $value; // TEMPORALMENTE: No escapar aquí, se hará en crear/actualizar
            }
        }    
        return $sanitizado;
    }
    ...
    public function crear() {
        ...
        $columnas = [];
        $valoresPlaceholders = []; // Para sentencias preparadas (ideal)
        $valoresParaBind = [];   // Para sentencias preparadas
        $tiposParaBind = '';     // Para sentencias preparadas

        $valoresDirectos = []; // Para la forma actual con escape_string

        foreach ($atributos as $key => $value) {
            $columnas[] = "`" . $key . "`"; // Encerrar nombres de columna en backticks

            if ($value === null) {
                $valoresDirectos[] = "NULL";
            } else {
                // Escapar AQUÍ, justo antes de construir la query
                $valoresDirectos[] = "'" . self::$conexion->escape_string($value) . "'";
            }
        }
    
        $columnasStr = join(', ', $columnas);
        $valoresStr = join(', ', $valoresDirectos);
        ...
        if($resultado) {
            ...
            // Para depuración si falla la inserción:
            self::setAlerta('error', 'Error al crear el registro: ' . self::$conexion->error);
            self::setAlerta('error', 'Query: ' . $query);
        }
        return $resultado;
    }
    ...
    public function actualizar(){
        // Obtener atributos SIN escapar en esta etapa (sanitizarAtributos modificado)
        $atributos = $this->sanitizarAtributos();
        ...
        foreach($atributos as $key => $value) {
            if ($value === null) {
                ...
                // Escapar AQUÍ, justo antes de construir la query
                $valoresSet[] = "`" . $key . "` = '" . self::$conexion->escape_string($value) . "'";
            }
        }

        $filas =  join(', ', $valoresSet);
        ...
        $resultado = self::$conexion->query($query);
         if(!$resultado) {
            // Para depuración si falla la actualización:
            // self::setAlerta('error', 'Error al actualizar el registro: ' . self::$conexion->error);
            // self::setAlerta('error', 'Query: ' . $query);
        }
        return $resultado;
    }
    ...
    public static function totalCondiciones($condiciones = []) {
        ...
        if (!empty($condiciones)) {
            $query .= " WHERE " . implode(' AND ', $condiciones); // Asume que las condiciones ya están escapadas si es necesario
        }
        $resultado = self::$conexion->query($query);
        $fila = $resultado->fetch_assoc(); // Usar fetch_assoc para obtener por nombre de columna
    ...
    public static function metodoSQL($params = []) {
        ...
        if (!empty($params['condiciones'])) {
            // Asume que las condiciones vienen listas o se escapan antes si es necesario
            $query .= " WHERE " . implode(' AND ', $params['condiciones']);
        }
        $query .= " ORDER BY " . $params['orden']; // Cuidado con inyección SQL en 'orden' si viene del usuario
        if ($params['limite']) {
            $query .= " LIMIT " . (int)$params['limite'];
            if ($params['offset']) {
